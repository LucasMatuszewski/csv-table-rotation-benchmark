                ...
                b.iter(|| {
                    ...
                    if let Value::Array(arr) = parsed_value {
                        let mut numbers: Vec<i64> =
                            arr.into_iter().map(|v| v.as_i64().unwrap()).collect();
...
fn bench_edge_cases(c: &mut Criterion) {
    ...
    for (name, data) in test_cases {
        group.bench_with_input(BenchmarkId::new("edge_case", name), &data, |b, input| {
            b.iter(|| {
                let mut data = input.clone();
...
fn bench_scaling(c: &mut Criterion) {
    ...
    for (n, label) in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("scale", label), &data, |b, input| {
            b.iter(|| {
                let mut data = input.clone();
                rotate_right(black_box(&mut data)).unwrap();
                std_black_box(data);
